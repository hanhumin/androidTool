...
public class CollapsedTextView extends AppCompatTextView implements View.OnClickListener {
     ...
     * 展示提示文本和图片
     * */
    private boolean showTipsTextAndDrawable = false;

    /**
     * 是否提示文本和图片一起显示
     * */
    public void setShowTipsTextAndDrawable(boolean showTipsTextAndDrawable) {
        this.showTipsTextAndDrawable = showTipsTextAndDrawable;
    }
     ...
     * 提示文本的点击事件
     */
    private CollapsedTextView.ExpandedClickableSpan mClickableSpan = new CollapsedTextView.ExpandedClickableSpan();
    /**
     * TextView的点击事件监听
     */
     ...
     */
    public void setTipsGravity(@CollapsedTextView.TipsGravityMode int tipsGravity) {
    ...
    public void setText(final CharSequence text, final TextView.BufferType type) {
        ...
        if (TextUtils.isEmpty(text) || mCollapsedLines == 0) {
            ...
            if (mShowWidth == 0||getMeasuredWidth()==0) {
...
//        }
    ...
    private void formatCollapsedText(TextView.BufferType type, CharSequence text) {
        ...
        if (line <= mCollapsedLines) {
            ...
            if(showTipsTextAndDrawable && mCollapsedDrawable != null){
                expandedTextWidth += mCollapsedDrawable.getIntrinsicWidth();
            }
            // 最后一行的宽
    ...
    private void setSpan(SpannableStringBuilder spannable) {
        ...
        if(showTipsTextAndDrawable && drawable != null){
            spannable.append(" ");
            spannable.setSpan(new ImageSpan(drawable, ImageSpan.ALIGN_BASELINE),spannable.length()-1,spannable.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
            spannable.append(" ");
        }else if (drawable != null) {// 如果提示的图片资源不为空，则使用图片代替提示文本
    ...
    private class ExpandedClickableSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            ...
            if (mTipsClickable) {
                ...
                if(showHideAllListener!=null)
                    showHideAllListener.fuckShowHide(mIsExpanded);
    ...
    public ShowHideAllListener showHideAllListener;

    public interface  ShowHideAllListener
    {
        void fuckShowHide(boolean showAll);
    }
}
