...
int usb_os_find_devices(struct usb_bus *bus, struct usb_device **devices)
{
  ...
  if (!dir)
  	{
	ALOGD("couldn't opendir(%s):  %s", dirpath,
	strerror(errno));
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (fd < 0) {
      fd = open(filename, O_RDONLY);
      if (fd < 0) {
	  	ALOGD("usb_os_find_devices: Couldn't open %s ", filename);
        if (usb_debug >= 2)
    ...
    if (ret < 0) {
		ALOGD("usb_os_find_devices: couldn't get connect info");
      if (usb_debug)
    ...
    if (ret < 0) {
		ALOGD("usb_os_find_devices: Couldn't read descriptor");
      if (usb_debug)
