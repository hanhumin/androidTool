...
struct usb_host_context *g_context = NULL;
...
{
    struct usb_host_context *context = (usb_host_context *)calloc(1, sizeof(struct usb_host_context));
...
retry:
    ...
    struct usb_device* result = (usb_device*)usb_device_new(dev_name, fd);
    if (result)
...
{
    struct usb_device *device = (usb_device *)calloc(1, sizeof(struct usb_device));
    int length;
...
{
    struct usbdevfs_urb *urb = (usbdevfs_urb *)calloc(1, sizeof(struct usbdevfs_urb));
    if (!urb)
        return NULL;
    ...
    struct usb_request *req = (usb_request *)calloc(1, sizeof(struct usb_request));
    if (!req) {
        free(urb);
        return NULL;
    }
